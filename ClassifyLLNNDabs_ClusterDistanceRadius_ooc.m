%% Classify using Absolute Large Margin Nearest Neighbor Distances with Cluster-specific Distance Radius Optimization
% The data space is reprojected using large margins. For each test sample,
% find the difference between the distance to the nearest neighbor, and the
% distance of the nearest neighbor to its nearest neighbor, to a cluster-specifc 
% distance radius. The distance radius itself is optimized by estimating
...
function [confusionTable, oocDetectionRate] = ClassifyLLNNDabs_ClusterDistanceRadius_ooc(data, labels, trainSplit)
