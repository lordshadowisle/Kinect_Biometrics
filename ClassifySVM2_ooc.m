%% Classify using Random Forest (out-of-class)
% Added functionality to perform evaluation using CV splits
% Simplified form
% 09/06: Added output for ooc detection statistics

function [confusionTable, oocDetectionRate] = ClassifySVM2_ooc(data, labels, trainSplit)
    ...
    for oocLabel = 1 : max(labels)
        ...
        for k = 1 : size(trainSplit,2)
            ...
            % Evaluation Task
            tempTrainSplit = trainSplit(:,k);
            [predictedLabel, ~, oneVsOne] = svmpredict(labels(~tempTrainSplit), data(~tempTrainSplit,:), learner, '-q');
            actualLabel = labels(~tempTrainSplit);
            scores = zeros(size(predictedLabel));
            % reject OOC samples
            for i = 1 : length(predictedLabel)
                tempOneVsOne = squareform(oneVsOne(i,:)) ;
                tempOneVsOne = tril(tempOneVsOne) - triu(tempOneVsOne);
                [tempScores] = sort(sum(tempOneVsOne), 'descend');
                scores(i) = tempScores(2) ./ tempScores(1);
            end
            OOCreject = scores > oocThreshold;
...
function [oocThreshold, correctMultiplier, wrongMultiplier] = SetThreshold(learner, data, labels)
    costFactor = 5;
    [predictedLabel, ~, oneVsOne] = svmpredict(labels, data, learner, '-q');
    ...
    for i = 1 : length(predictedLabel)
        ...
        tempOneVsOne = tril(tempOneVsOne) - triu(tempOneVsOne);
        [tempScores] = sort(sum(tempOneVsOne), 'descend');
        multiplier(i) = tempScores(2) ./ tempScores(1);
    end
