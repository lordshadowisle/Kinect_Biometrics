...
function [confusionTable, oocDetectionRate] = ClassifyRandomForest_ooc(data, labels, trainSplit)
    ...
    for oocLabel = 1 : max(labels)
        ...
        for k = 1 : size(trainSplit,2)
            ...
            learner = fitensemble(data(tempTrainSplit,:), labels(tempTrainSplit), 'Bag', 100, 'Tree', 'type', 'classification');
